...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE
        embedding = tf.Variable(self.pretrained_embeddings, dtype=tf.float32)
        emb = tf.nn.embedding_lookup(embedding, self.input_placeholder)
        embeddings = tf.reshape(emb, [-1, self.config.n_features * self.config.embed_size])
        ### END YOUR CODE
        return embeddings
